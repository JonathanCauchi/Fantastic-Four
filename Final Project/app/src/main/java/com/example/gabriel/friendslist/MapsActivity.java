...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mFriendDatabase = FirebaseDatabase.getInstance().getReference().child("Friends").child(currentUserId);
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        myReqTicketRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot){
                ...
                if (!rt.status.equals("Pending") && !rt.receivingLocationFrom.equals("EmptyTicket")) {

                    mUserDatabase.child(rt.receivingLocationFrom).child("userName").addValueEventListener(new ValueEventListener() {
                        @Override
                        public void onDataChange(DataSnapshot dataSnapshot) {
                            ...
                            if (rt.status.equals("Approved")) {
                                if(trackingLatLng != null){
                                    updateTrackingMarker(new LatLng(trackingLatLng.latitude, trackingLatLng.longitude), /*rt.receivingLocationFrom*/userName);
        ...
        final TextView nav_user = hView.findViewById(R.id.username_text);
        final ImageView nav_pic = hView.findViewById(R.id.profile_image);

        mMyUserDatabase.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {

                String userName = dataSnapshot.getValue().toString();

                storageRef.child(currentUserId).getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                    @Override
                    public void onSuccess(Uri uri) {
                        Picasso.with(MapsActivity.this).load(uri).fit().centerCrop().into(nav_pic);
                    }
                }).addOnFailureListener(new OnFailureListener() {
                    @Override
                    public void onFailure(@NonNull Exception exception) {
                        Toast.makeText(MapsActivity.this, "Failed to get Profile Picture", Toast.LENGTH_SHORT).show();
                    }
                });
        ...
        navigationView.setNavigationItemSelectedListener(
                new NavigationView.OnNavigationItemSelectedListener() {
                    ...
                    public boolean onNavigationItemSelected(MenuItem menuItem) {
                        ...
                        else if(menuItem.getTitle().equals("Friends")) {
    ...
    private void genReqNotification(Request request){
        ...
        mUserDatabase.child(fRequest.from).child("userName").addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
            ...
            for (int i = 0; i < nearby.size(); i++) {
                ...
                if (latLng.latitude - myLatLng.latitude <= 1 && latLng.longitude - myLatLng.longitude <= 1 && latLng.latitude - myLatLng.latitude >= -1 &&latLng.longitude - myLatLng.longitude >= -1) {
                    if(!nearbyTemp.userId.equals(currentUserId)&&!nearbyTemp.userId.equals("00")) {
                        ...
                        mUserDatabase.child(nearbyTemp.userId).child("userName").addValueEventListener(new ValueEventListener() {
                            @Override
                            public void onDataChange(DataSnapshot dataSnapshot) {
                            ...
                            public void onDataChange(DataSnapshot dataSnapshot) {
                                ...
                                String friend_id = mFriendDatabase.getRef().getKey();
